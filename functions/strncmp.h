...
#  define REPLAY_ITERATION(rec) \
  if (rec.n < 1000000) \
    { \
      ...
      ret += (uintptr_t) strncmp (start1, start2, rec.capa); \
      start1[rec.n] = 1; \
    }
# endif
...
#undef strncmp
int
strncmp (const char *_src, const char *_src2, size_t capa)
{
  ...
  for (i = 0; i < capa && src[i] && src[i] == src2[i]; i++)
    ;
